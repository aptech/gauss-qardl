...
| ARDL GETS selection | `gets_ardl_uecm()` performs general-to-specific reduction with `gets.lm`. | `ardlFull()`, `ardlSelect()`, and scalar `pqSelect` accept `criterion = "gets"` and `gets_pval`; `ardlSparseGETS()` exposes standalone sparse Case V UECM pruning; `ardlAutoCase(..., gets_mode = "sparse")` keeps sparse pruning inside auto-case; R-package validation exports sparse GETS keep/coefficient diagnostics. | Broad partial | GAUSS now covers hierarchical lag-block GETS, standalone sparse UECM output, sparse auto-case UECM pruning, and optional final-selection comparison against `gets_ardl_uecm()`. Exact deletion-order trace parity remains deferred unless users need a step-by-step audit trail. |
...
| Fixed-order NARDL unrestricted ECM | `nardl_uecm()` returns `NARDL_ECM_fit` and UECM summaries. | `nardlECM(..., ecm_type = "uecm", case_id = 1..5, d, thresh1, thresh2)`. | Partial | Public GAUSS support exists with deterministic cases and threshold reset controls; exact R sample/parameterization parity remains a validation target. |
...
| NARDL GETS selection | `gets_nardl_uecm()` and `nardl_auto_case()` perform general-to-specific reduction. | `nardlFull()`, `ardlSelect(model = "nardl")`, and `nardlOrder()` accept `criterion = "gets"` and `gets_pval`; `nardlSparseGETS()` exposes standalone sparse Case V NARDL UECM pruning; `nardlAutoCase(..., gets_mode = "sparse")` keeps sparse pruning inside auto-case; R-package validation exports sparse GETS keep/coefficient diagnostics. | Broad partial | GAUSS now covers hierarchical lag-block GETS, standalone sparse UECM output, sparse auto-case UECM pruning, and optional final-selection comparison against `gets_nardl_uecm()`. Exact deletion-order trace parity remains deferred unless users need a step-by-step audit trail. |
...
| QARDL lag selection/full workflow | Not in `ardl.nardl`. | `qardlFull()`, `ardlSelect(model = "qardl")`, and documented `pqSelect`; legacy `pqorder*` and `pqorderX*` aliases remain callable. | GAUSS-only | Includes IC selection, scalar GETS selection, restricted grids, and per-regressor lag vector support. |
...
| CS-ARDL panel estimation | Not in `ardl.nardl`. | `csardl()`, `csardlECM(..., ecm_type)`, `csardlFull()`, `ardlSelect(model = "csardl")`, `csardlOrder*`. | GAUSS-only | Major GAUSS extension into panel ARDL. |
...
| ---: | --- | --- |
| 1 | Promote sparse GETS diagnostics to hard parity checks when conventions are confirmed | The validation harness now compares final sparse GETS keep vectors, coefficients, `sigma2`, nobs, and dropped counts against `gets_ardl_uecm()` / `gets_nardl_uecm()` when R is available. The remaining work is deciding whether final-selection parity is sufficient or whether a step-by-step deletion trace should be added. |
