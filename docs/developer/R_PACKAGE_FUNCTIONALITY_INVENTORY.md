...
| ARDL automatic case workflow | `auto_case_ardl()` searches case/specification workflow. | `ardlAutoCase()` selects p/q by hierarchical GETS, infers the admissible PSS case set from Case V UECM deterministic terms, and reports case-specific bounds rows. | Partial | GAUSS now has the wrapper and case-set reporting; R still uses sparse `gets.lm` term deletion. |
...
| Fixed-order NARDL unrestricted ECM | `nardl_uecm()` returns `NARDL_ECM_fit` and UECM summaries. | `nardlECM(..., ecm_type = "uecm", case_id = 1..5, d, thresh1, thresh2)`. | Partial | Public GAUSS support exists with deterministic cases and threshold reset controls; R package parity still needs sparse-term GETS alignment. |
...
| NARDL automatic case workflow | `nardl_auto_case()` combines NARDL, GETS, and case selection. | `nardlAutoCase()` combines hierarchical GETS, named NARDL decomposition/control specs, threshold reset options, Case V deterministic-term inference, and case-specific bounds rows. | Partial | GAUSS now has the wrapper and case-set reporting; R still uses sparse `gets.lm` term deletion. |
...
| QARDL lag selection/full workflow | Not in `ardl.nardl`. | `qardlFull()`, `pqorder*`, `pqorderX*`. | GAUSS-only | Includes IC selection, scalar GETS selection, and per-regressor lag vector support. |
...
| ---: | --- | --- |
| 1 | Sparse-term GETS parity | GAUSS supports hierarchical GETS p/q order reduction and auto-case wrappers; R's `gets_*` can return arbitrary parsimonious UECM subsets through `gets.lm`. |
