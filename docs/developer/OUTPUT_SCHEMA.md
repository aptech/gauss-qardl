...
| `ardlFullOut` | ARDL | common workflow metadata, `pmax`, `qmax`, `levels_diag` | Bundles selected ARDL output in `.ar` and residual diagnostics in `.levels_diag`. |
...
| `qardlFullOut` | QARDL | common workflow metadata, `pmax`, `qmax`, `levels_diag`, `ecm_diag` | Propagates formula/name metadata to `.qa` and `.ecm` and stores residual diagnostics for both selected estimators. |
| `qardlWaldOut` | QARDL Wald tests | `model_family`, `test`, `family`, `tau`, `wald`, `pv`, family-specific Wald/p-value fields, optional `bigR`/`smr` | Returned by `qardlWald`; built-in constancy and symmetry tests store beta/gamma/phi rows, while custom tests store the generated restriction matrices. |
...
| `nardlFullOut` | NARDL | common workflow metadata, `pmax`, `qmax`, `decomp_thresholds`, `levels_diag`, `ecm_diag` | Propagates formula/name metadata to `.na` and `.ecm` and stores residual diagnostics for both selected estimators. |
...
| `csardlFullOut` | CS-ARDL | common workflow metadata, `unitvar`, `timevar`, `pmax`, `qmax`, `panel_diag` | Propagates formula/name metadata to `.csa` and `.ecm` and stores selected-specification panel diagnostics in `.panel_diag`. |
