...
1) wald test for testing long-run parameter;
...
1) beta (b): the estimated long-run parameter by qardl.prc;
...
proc (2) = wtestlrb(beta,cov,bigR,smr,data);
    local nn, wt, rnk, pv;
    nn = rows(data);
    wt = ((nn-1)^2)*(bigR*beta-smr)'*inv(bigR*cov*bigR')*(bigR*beta-smr);
    rnk= rows(bigR);
    pv = cdfchic(wt,rnk);
    retp(wt, pv);
endp;